...
export class PollController {
  ...
  async createPoll(@Body() createPollDto: CreatePollDto, @Request() req): Promise<PollResponseDto> {
    if (!createPollDto.options || createPollDto.options.length < 2 || createPollDto.options.length > 4) {
  ...
  @Delete(':id')
  @UseGuards(JwtAuthGuard, AdminGuard)
  async deletePoll(@Param('id') pollId: string, @Request() req): Promise<{ message: string }> {
  ...
  @Patch(':id/close')
  @UseGuards(JwtAuthGuard, AdminGuard)
  async closePoll(@Param('id') pollId: string, @Request() req): Promise<PollResponseDto> {
